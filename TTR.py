...
def generate_bar_plot(orig_hit_file, orig_reached_file, work_hit_file, work_reached_file, use_ttr_limit, output_filename):
    orig_target_time = parse_target_reached(orig_reached_file) if use_ttr_limit else None
    work_target_time = parse_target_reached(work_reached_file) if use_ttr_limit else None
    ...
    # Sort primarily by Without Control (orig_ctrl / blue) time, then by With Control (work_ctrl / orange) time
    ...
    ax1.set_xticklabels([str(i) for i in control_ids], rotation=45, fontsize=8)
    ...
    # Sort primarily by Without Control (orig_call / blue) time, then by With Control (work_call / orange) time
    ...
    ax2.set_xticklabels([str(i) for i in caller_ids], rotation=45, fontsize=8)
...
def main():
    ...
    generate_bar_plot(orig_hit_file, orig_reached_file, work_hit_file, work_reached_file,
