    ...
    for method in methods:
        ...
        for item in trial_items:
            ...
            if not exposure_file_path:
                exposure_file_path = os.path.join(local_trial_dir, "tte_queue.txt") # Default fallback if missing
            ...
            crashes_dirs = []
            out_dir = os.path.join(local_trial_dir, "out")
            if os.path.exists(out_dir):
                for node_name in os.listdir(out_dir):
                    node_crashes_dir = os.path.join(out_dir, node_name, "queue")
                    if os.path.isdir(node_crashes_dir):
                        crashes_dirs.append((node_name, node_crashes_dir))
            ...
            for name, local_crashes_dir in crashes_dirs:
                ...
                dest_logs_dir = os.path.join(artifact_dir, item["session_dir"], "TTE_check", f"{method}_{item['trial']}_{name}_queue_full_logs")
